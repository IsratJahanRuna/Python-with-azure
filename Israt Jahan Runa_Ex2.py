...
sortedAggregatedDF = aggregatedDF.orderBy(F.desc("year"))


sortedAggregatedDF.show()
...
filteredDF = weatherDataFrameWithYear.filter((F.col("fog") == "Yes"))

fogCountDF = filteredDF.groupBy("year").agg(F.count("date").alias("fog_days_count"))

yearWithMostDaysWithFog = fogCountDF.orderBy(F.desc("fog_days_count")).select("year").first()[0]

print(f"The year with the highest number of fog days (excluding heavy fog): {yearWithMostDaysWithFog}")
